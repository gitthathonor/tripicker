...
public class PlanSpotInsertAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String sigungucode = request.getParameter("sigungucode");
			...
			urlBuilder.append("&" + URLEncoder.encode("arrange", "UTF-8") + "=" + URLEncoder.encode("A",
					"UTF-8")); /*
								 * (A=제목순, B=조회순, C=수정일순, D=생성일순) , 대표이미지가 반드시 있는
								 * 정렬 (O=제목순, P=조회순, Q=수정일순, R=생성일순)
								 */
			...
			urlBuilder.append("&" + URLEncoder.encode("areaCode", "UTF-8") + "="
					+ URLEncoder.encode(areacode, "UTF-8")); /* 지역코드 */
			...
			urlBuilder.append("&" + URLEncoder.encode("cat3", "UTF-8") + "=" + URLEncoder.encode(cat3,
					"UTF-8")); /* 소분류 코드(cat1,cat2필수) */
			...
			try {
				...
				System.out.println("파싱할 리스트 수 : " + items.getLength());// 파싱할
																		// 리스트 수
		 ...
		 * 
		 * 
		 * // JSONObject jobj = new JSONObject(); // jobj.get(areaList);
		 * 
